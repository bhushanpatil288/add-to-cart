import { fireToast, addToCart, renderProducts, alreadyExists, updateBadge, toyCardsContainer, countBadge } from "./utils/utils.js";
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  catFilterBtn.addEventListener("click", (e) => {
    ...
    if (!catFilter) {
      renderProducts(products);
      fireToast("info", "Category filter removed");
      return;
    }
    const FilteredProducts = products.filter(
      (product) => product.category === categories_filter[catFilter]
    );
    renderProducts(FilteredProducts);
    fireToast("success", "Category filter applied");
  });
  ...
  minMaxFilterBtn.addEventListener("click", (e) => {
    ...
    const FilteredProducts = products.filter(
      (product) =>
        product.price >= minimumValue && product.price <= maximumValue
    );
    renderProducts(FilteredProducts);
    fireToast("success", "Price filter applied");
  });
});
